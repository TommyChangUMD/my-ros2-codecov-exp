...
public:
  ...
  void topic_callback(const std_msgs::msg::String& msg)
  {
    RCLCPP_INFO(node_->get_logger(),
                "CallbackFunction id=%d: I heard: '%s'",
                id,
                msg.data.c_str());
...
public:
    ...
    // count_(0)
  {
    ...
    // subscription_ = this->create_subscription<String>(topic_name, 10,
    // callback);

    for (int idx = 0; idx < numNodes; idx++)
    {
      std::string nodeName = "my_node" + std::to_string(idx);
      cbFunts_[idx] = std::make_shared<CallbackFunction>(
        static_cast<RCL_NODE_PTR>(this),
        idx);

      auto callback = std::bind(&CallbackFunction::topic_callback,
                                cbFunts_[idx],
                                _1);
